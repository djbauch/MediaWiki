 ...
 * @since 1.17
 */
class SqlserverInstaller extends DatabaseInstaller {
	...
	var $minimumVersion = '2005';
	var $maxRoleSearchDepth = 5;

	protected $sqlConns = array();
	...
	function submitConnectForm() {
		...
		$status = $this->getSqlConnection( 'create-db' );
		if ( !$status->isOK() ) {
			return $status;
		}
		$conn = $status->value;
	...
	public function getConnection() {
		$status = $this->getSqlConnection( 'create-tables' );
		if ( $status->isOK() ) {
	...
	public function openConnection() {
		return $this->openSqlConnection( 'create-tables' );
	}
	 ...
	 * @return Status
	 */
	protected function getSqlConnection( $type ) {
		if ( isset( $this->sqlConns[$type] ) ) {
			return Status::newGood( $this->sqlConns[$type] );
		}
		$status = $this->openSqlConnection( $type );

		if ( $status->isOK() ) {
			$conn = $status->value;
			$conn->clearFlag( DBO_TRX );
			$conn->commit();
			$this->sqlConns[$type] = $conn;
		}
		return $status;
	}

	/**
	 * Get a connection of a specific QL-specific type. Connections
	 ...
	 *   value member.
	 */
	protected function openSqlConnection( $type ) {
		switch ( $type ) {
			case 'create-db':
			...
			case 'create-tables':
				$status = $this->openSqlConnection( 'create-schema' );
				if ( $status->isOK() ) {
					$conn = $status->value;
...
//		return $row;
	...
	protected function canCreateAccounts() {
		return true;
//		$row = $conn->select( '[sys].[server_principals]', '*',
//			array( 'name' => $superuser ), __METHOD__ );
...
//			array( 'roleid' => $group ), __METHOD__ );
	...
	function setupDatabase() {
		...
		$exists = $conn->selectField( '[sys].[databases]', '1',
			array( 'name' => $dbName ), __METHOD__ );
		if ( !$exists ) {
	...
	function setupSchema() {
		...
		$status = $this->getSqlConnection( 'create-schema' );
		if ( !$status->isOK() ) {
			return $status;
		}
		$conn = $status->value;
...
//		$exists = $conn->selectField( '"pg_catalog"getSqlConnectionge"', 1,
